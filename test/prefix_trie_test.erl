...
-import(prefix_trie, [new/0, insert/3, get/2, remove/2, contains/2, filter/2, map/2]).

-include_lib("eunit/include/eunit.hrl").

prefix_trie_test() ->
  % Test empty trie
  ?assertEqual(false, prefix_trie:contains([], new())),

  % Test single value
  T1 = prefix_trie:insert("hello", 50, new()),
  ?assertEqual(true, prefix_trie:contains("hello", T1)),
  ?assertEqual(false, prefix_trie:contains("world", T1)),

  % Test multiple values
  T2 = prefix_trie:insert("world", 100, T1),
  T3 = prefix_trie:insert("foo", 200, T2),
  T4 = prefix_trie:insert("bar", 300, T3),
  T5 = prefix_trie:insert("foobar", 400, T4),
  T6 = prefix_trie:insert("foofoo", 500, T5),
  ...
  Pred = fun(X) -> X > 300 end,
  FilteredTrie = prefix_trie:filter(Pred, T6),
  ...
  ?assertEqual(false, prefix_trie:contains("bar", FilteredTrie)),
  ?assertEqual(true, prefix_trie:contains("foobar", FilteredTrie)),
  ?assertEqual(true, prefix_trie:contains("foofoo", FilteredTrie)),

  %% Test map
  Fun = fun(X) -> X * 2 end,
  T11 = prefix_trie:map(Fun, T6),
  ?assertEqual({ok, 200}, prefix_trie:get("world", T11)),
  ?assertEqual({ok, 400}, prefix_trie:get("foo", T11)),
  ?assertEqual({ok, 600}, prefix_trie:get("bar", T11)),
  ?assertEqual({ok, 800}, prefix_trie:get("foobar", T11)),
  ?assertEqual({ok, 1000}, prefix_trie:get("foofoo", T11)),
